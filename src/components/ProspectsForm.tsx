...
export function ProspectsForm({ salesReps, onSubmit, isLoading = false }: ProspectsFormProps) {
  ...
  useEffect(() => {
    const initializeExpansionState = async () => {
      try {
        ...
        if (email) {
          const storageKey = `prospectsFormExpanded_${email}`;
  ...
  const toggleExpand = useCallback(() => {
    setIsExpanded(prev => {
      const newState = !prev;
      if (userEmail) {
        const storageKey = `prospectsFormExpanded_${userEmail}`;
    ...
    <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm">
      ...
      <div className={`p-6 space-y-6 transition-all duration-200 ease-in-out ${
        ...
        <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
          ...
          <button
            type="submit"
            disabled={isLoading || !selectedFile || !targetAccount.trim() || !assignedTo}
