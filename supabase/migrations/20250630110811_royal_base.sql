...
-- Ajouter une contrainte unique sur l'email (sans IF NOT EXISTS)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'sales_reps_email_unique' 
    AND table_name = 'sales_reps'
  ) THEN
    ALTER TABLE sales_reps ADD CONSTRAINT sales_reps_email_unique UNIQUE (email);
  END IF;
END $$;
