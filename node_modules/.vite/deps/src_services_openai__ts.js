...
var SYSTEM_PROMPT = `Tu es un assistant spécialisé dans l'analyse d'appels d'offres (AO) pour des missions de consulting IT.
...
var PROSPECT_SYSTEM_PROMPT = `Tu es un assistant spécialisé dans l'analyse de profils de candidats pour des missions de consulting IT.
...
async function analyzeRFP(content) {
  ...
  try {
    const { supabase } = await import("./supabase-PSP3SS3H.js");
  ...
  try {
    ...
    let result;
    try {
      const content2 = data.choices[0].message.content.replace(/``\`json\n|\n```/g, "");
      result = JSON.parse(content2);
    } catch (error) {
...
async function analyzeProspect(content, cvContent) {
  var _a;
  let apiKey = localStorage.getItem("openai-api-key") || "";
  try {
    const { supabase } = await import("./supabase-PSP3SS3H.js");
  ...
  const fullContent = cvContent ? `INFORMATIONS TEXTUELLES:
${content}
...
//# sourceMappingURL=src_services_openai__ts.js.map
