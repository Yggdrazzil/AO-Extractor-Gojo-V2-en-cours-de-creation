  ...
  useEffect(() => {
    ...
    const initializeApp = async () => {
      try {
        ...
        if (currentSession) {
          // Étape 1.5: Initialiser le système de cron côté client
          try {
            console.log('⏰ Initializing client-side cron system...');
            await initializeClientSideCron();
            console.log('✅ Client-side cron system ready');
          } catch (cronError) {
            console.warn('⚠️ Client-side cron initialization failed (non-blocking):', cronError);
          }
