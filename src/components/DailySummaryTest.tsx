...
import { triggerDailySummary, getDailySummaryStats, checkCronStatus } from '../services/dailySummary';
import type { DailySummaryResult } from '../services/dailySummary';
...
export function DailySummaryTest() {
  ...
  const handleTriggerSummary = async () => {
    ...
    try {
      const summaryResult = await triggerDailySummary();
  ...
  const handleLoadStats = async () => {
    ...
    try {
      const statsResult = await getDailySummaryStats();
  ...
  const handleCheckCron = async () => {
    ...
    try {
      const cronResult = await checkCronStatus();
      setCronStatus(cronResult);
    } catch (err) {
      console.error('Error checking cron:', err);
    ...
    <div className="space-y-6">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4 flex items-center gap-2">
          ...
          Test du récapitulatif quotidien
        </h3>
              ...
              <div className="flex items-start gap-2 mb-3">
                ...
                <div>
                  <h4 className="font-medium text-green-800 dark:text-green-200">
                    Récapitulatif envoyé avec succès
                  </h4>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400">
                      AO{stat.pendingRFPs > 1 ? 's' : ''} à traiter
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono text-sm text-gray-900 dark:text-gray-100">
                Tous les jours à 9h00 (heure française)
              </span>
            </div>
