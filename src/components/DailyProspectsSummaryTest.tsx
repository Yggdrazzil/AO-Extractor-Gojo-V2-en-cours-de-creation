...
import { triggerDailyProspectsSummary, getDailyProspectsSummaryStats, checkProspectsCronStatus } from '../services/dailyProspectsSummary';
import type { DailyProspectsSummaryResult } from '../services/dailyProspectsSummary';

export function DailyProspectsSummaryTest() {
  const [isLoading, setIsLoading] = useState(false);
  const [result, setResult] = useState<DailyProspectsSummaryResult | null>(null);
  ...
  const handleTriggerSummary = async () => {
    ...
    try {
      const summaryResult = await triggerDailyProspectsSummary();
      setResult(summaryResult);
    } catch (err) {
      console.error('Error triggering prospects summary:', err);
  ...
  const handleLoadStats = async () => {
    ...
    try {
      const statsResult = await getDailyProspectsSummaryStats();
      setStats(statsResult);
    } catch (err) {
      console.error('Error loading prospects stats:', err);
  ...
  const handleCheckCron = async () => {
    ...
    try {
      const cronResult = await checkProspectsCronStatus();
      setCronStatus(cronResult);
    } catch (err) {
      console.error('Error checking prospects cron:', err);
    ...
    <div className="space-y-6">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4 flex items-center gap-2">
          ...
          Test du récapitulatif quotidien des prospects
        </h3>
              ...
              <div className="flex items-start gap-2 mb-3">
                ...
                <div>
                  <h4 className="font-medium text-green-800 dark:text-green-200">
                    Récapitulatif prospects envoyé avec succès
                  </h4>
                    ...
                    <span className="flex items-center gap-2">
                      {res.emailSent ? (
                        <>
                          ...
                          {res.pendingProspects} prospect(s)
                        </>
                      ) : (
                        <>
      ...
      {stats && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4 flex items-center gap-2">
            <Users className="w-5 h-5" />
            Statistiques prospects par commercial
          </h3>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">
                      {stat.pendingProspects}
                    </div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">
                      Prospect{stat.pendingProspects > 1 ? 's' : ''} à traiter
                    </div>
                  </div>
                </div>
              </div>
      ...
      {cronStatus && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4 flex items-center gap-2">
            ...
            Configuration automatique prospects
          </h3>
